...
from strl.rl.agents.ac_agent import ACAgent
from strl.rl.components.agent import BaseAgent
from strl.utils.general_utils import ParamDict, map_dict, AttrDict
from strl.utils.pytorch_utils import ten2ar, avg_grad_norm, TensorModule, check_shape, map2torch, map2np
from strl.rl.utils.mpi import sync_networks


class DiscreteSACAgent(ACAgent):
    """Implements Discrete SAC algorithm."""
    ...
    def update(self, experience_batch):
        ...
        for _ in range(self._hp.update_iterations):
            ...
            with torch.no_grad():
                ...
                reward = experience_batch.reward.view(self._hp.batch_size, 1)
                done = experience_batch.done.view(self._hp.batch_size, 1)
                q_target = reward * self._hp.reward_scale + (1 - done) * self._hp.discount_factor * value_next
                if self._hp.clip_q_target:
                    q_target = self._clip_q_target(q_target)
                q_target = q_target.detach()
                check_shape(q_target, [self._hp.batch_size, self._hp.critic_params.output_dim])
    ...
    def _compute_alpha_loss(self, policy_output):
        return -1 * (policy_output.dist.prob.probs.detach() * self.alpha * (
                self._target_entropy + policy_output.dist.prob.logits).detach()).mean()

    def _compute_policy_loss(self, experience_batch, policy_output):
        q_est = torch.min(*[critic(experience_batch.observation).q
                            for critic in self.critics])
        policy_loss = policy_output.dist.prob.probs * (-1 * q_est + self.alpha * policy_output.dist.prob.logits)
    ...
    def _compute_next_value(self, experience_batch, policy_output):
        q_next = torch.min(
            *[critic_target(experience_batch.observation_next).q for critic_target in self.critic_targets])
        next_val = policy_output.dist.prob.probs * (q_next - self.alpha * policy_output.dist.prob.logits)
    ...
    def _compute_critic_loss(self, experience_batch, q_target):
        ...
        check_shape(qs[0], [self._hp.batch_size, self._hp.critic_params.output_dim])
        critic_losses = [0.5 * (q - q_target).pow(2).mean() for q in qs]
        return critic_losses, qs

    def _compute_q_estimates(self, experience_batch):
        return [critic(experience_batch.observation).q.squeeze(-1)
