...
from skilltree.utils.general_utils import AttrDict, RecursiveAverageMeter, ParamDict
...
class ReplayBuffer:
    ...
    def append(self, experience_batch):
        ...
        for key in self._replay_buffer:
            self._replay_buffer[key][idxs] = np.asarray(experience_batch[key])
...
class PrioritizedReplayBuffer(ReplayBuffer):
    def sample(self, n_samples, filter=None):
        assert n_samples <= self.size  # need enough samples in replay buffer
        assert isinstance(self.size, int)  # need integer-valued size
        assert 'prob' in self._replay_buffer
        idxs = torch.multinomial(self._replay_buffer['prob'], n_samples, replacement=True)
        # idxs = np.random.choice(np.arange(self.size), size=n_samples)
...
class SplitObsReplayBuffer(ReplayBuffer):
    ...
    def append(self, experience_batch):
        ...
        if self._hp.discard_part == 'front':
            filtered_experience_batch.observation = [o[self._hp.unused_obs_size:] for o in
                                                     filtered_experience_batch.observation]
            filtered_experience_batch.observation_next = [o[self._hp.unused_obs_size:] for o in
                                                          filtered_experience_batch.observation_next]
