...
import numpy as np
import torch
from skilltree.utils.general_utils import str2int
...
class CheckpointHandler:
    ...
    @staticmethod
    def load_weights(weights_file, model, load_step=False, load_opt=False, optimizer=None, strict=True, opt=None):
        success = False
        if os.path.isfile(weights_file):
            ...
            if opt is not None:
                opt_checkpoint = {}
                for k, v in checkpoint['state_dict'][opt].items():
                    if 'policy.net' in k:
