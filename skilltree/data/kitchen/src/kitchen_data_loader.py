...
from skilltree.components.data_loader import Dataset
...
class D4RLImageSequenceSplitDataset(Dataset):
    SPLIT = AttrDict(train=0.99, val=0.01, test=0.0)
    ...
    def __init__(self, data_dir, data_conf, phase, resolution=None, shuffle=True, dataset_size=-1):
        ...
        self.dataset = np.load(self._load_npz_files(data_dir)[0])
        # self.dataset = np.memmap(self._load_npz_files(data_dir)[0], dtype='float32', mode='r')
        ...
        for end_idx in seq_end_idxs:
            if end_idx + 1 - start < self.subseq_len: continue  # skip too short demos
            self.seqs.append(AttrDict(
                states=observations[start:end_idx + 1],
                actions=actions[start:end_idx + 1],
            ))
            start = end_idx + 1
    ...
    def __len__(self):
        if self.dataset_size != -1:
            return self.dataset_size
        return int(self.SPLIT[self.phase] * self.dataset['pixel_ob'].shape[0] / self.subseq_len)
