...
class HPRolloutSaver(object):
    ...
    def save(self, file_name, save_interval=100, reset=False):
        if self.num_episode > 0 and self.num_episode % save_interval == 0:
            save_path = os.path.join(self.save_dir, f"{file_name}.h5")
...
class SERolloutSaver(object):
    ...
    def save_rollout(self, episode):
        ...
        self.data['observation'] = np.array(episode['observation'])
        self.data['hl_action_index'] = np.array(episode['hl_action_index'])
        # self.data['complete_task'] = np.array(episode["complete_task"])
