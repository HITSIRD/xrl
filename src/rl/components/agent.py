...
from src.utils.general import ParamDict, get_clipped_optimizer, AttrDict, prefix_dict, map_dict, \
    nan_hook, np2obj, ConstantSchedule, attr_to_batch
from src.utils.pytorch import (RAdam, remove_grads, map2np, map2torch)
# from src.utils.vis import add_caption_to_img, add_captions_to_seq
# from src.rl.components.normalization import DummyNormalizer
from src.rl.components.policy import Policy
from src.components.checkpointer import CheckpointHandler


# from src.rl.utils.mpi import sync_grads
...
class BaseAgent(nn.Module):
    ...
    def act(self, obs, index=None):
        ...
        if self._rand_act_mode:
            ...
            return self._act(obs, index=index)
    ...
    @staticmethod
    def load_model_weights(model, checkpoint, epoch='latest', opt=None):
        ...
        CheckpointHandler.load_weights(checkpoint_path, model=model, opt=opt)
...
class HierarchicalAgent(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self._hp.hl_agent_params.device = config.device
        self._hp.ll_agent_params.device = config.device
        self.hl_agent = self._hp.hl_agent(self._hp.hl_agent_params)
        self.ll_agent = self._hp.ll_agent(self._hp.ll_agent_params)
    ...
    def act(self, obs, index=None, task=None, env=None):
        ...
        if not isinstance(obs, AttrDict):
            ...
            obs_input = attr_to_batch(obs) if len(obs.states.shape) == 1 else obs

        output = AttrDict()
        if self._perform_hl_step_now:
            # perform step with high-level policy
            self._last_hl_output = self.hl_agent.act(obs_input, index=index)
            output.is_hl_step = True

            if isinstance(obs_input, AttrDict):
                if len(self._last_hl_output.action.shape) == 1:
                    self._last_hl_output.action = self._last_hl_output.action[None]  # add batch dim if necessary
            elif len(obs_input.shape) == 2 and len(self._last_hl_output.action.shape) == 1:
                self._last_hl_output.action = self._last_hl_output.action[None]  # add batch dim if necessary
        else:
        ...
        if isinstance(obs, AttrDict):
            return self._remove_batch(output) if len(obs.states.shape) == 2 else output
        return self._remove_batch(output) if len(obs.shape) == 1 else output

    def update(self, experience_batches, info):
        ...
        if self._hp.update_hl:
            hl_update_outputs = self.hl_agent.update(experience_batches.hl_batch, info)
...
class SubtaskIntervalHierarchicalAgent(HierarchicalAgent):
    def __init__(self, config):
        super().__init__(config)
        self._hl_step = True
    ...
    def perform_hl_step(self):
        self._hl_step = True

    @property
    def _perform_hl_step_now(self):
        return self._hl_step

    def reset(self):
        super().reset()
        self._hl_step = True  # start new episode with high-level step
