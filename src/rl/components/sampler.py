...
from src.utils.general import listdict2dictlist, AttrDict, ParamDict, obj2np


# from src.modules.variational_inference import MultivariateGaussian
# from src.rl.utils.reward_fcns import sparse_threshold
...
class HierarchicalSampler(Sampler):
                    ...
                    while hl_step < batch_size:
                        ...
                        if store_ll:
                            if ll_experience_batch:
                                ll_experience_batch[-1].observation_next = \
                                    self._agent.make_ll_obs(ll_experience_batch[-1].observation_next,
                                                            agent_output.hl_action)
                        ...
                        if agent_output.is_hl_step or (done or self._episode_step >= self._max_episode_len - 1):
                            if self.last_hl_obs is not None and self.last_hl_action is not None:
                                ...
                                self._postprocess_experience(hl_experience, agent_output)
                                hl_experience_batch.append(hl_experience)
                                hl_step += 1
                                self._episode_start = False
                                if done:
                                    hl_experience_batch[-1].reward += reward  # add terminal reward
                                batch_info.obs = obs
                                batch_info.done = done
                        ...
                        if done or self._episode_step >= self._max_episode_len:
                            ...
                            self._episode_reset(step)

        return AttrDict(
            hl_batch=listdict2dictlist(hl_experience_batch),
            ll_batch=listdict2dictlist(ll_experience_batch[:-1]) if store_ll else None,
        ), env_steps, batch_info
    ...
    def _postprocess_experience(self, experience, agent_output):
        if hasattr(agent_output, 'hl_action_index'):
            experience.action_index = agent_output.hl_action_index
        if hasattr(agent_output, 'hl_log_prob'):
            experience.log_prob = agent_output.hl_log_prob
        if hasattr(agent_output, 'hl_value'):
            experience.value = agent_output.hl_value
...
class ImageAugmentedSampler(Sampler):
    ...
    def _postprocess_obs(self, obs):
        img = self._env.render().astype(np.float32).transpose(2, 0, 1) * 2. - 1.0
